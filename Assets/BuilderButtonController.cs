...
public class BuilderButtonController : MonoBehaviour {
    ...
    public void toggleButton(int buttonIndex) {
        ...
        {
            saveState();
            foreach (GameObject node in GameObject.FindGameObjectsWithTag("BuilderAminoAcid"))
            {
                Destroy(node);
            }
            GameObject.FindGameObjectWithTag("ProteinBuildArea").GetComponent<ProteinBuildSpawn>().spawnProteinBuild(buttons[buttonIndex].GetComponent<BuilderButton>().proteinBuild);
